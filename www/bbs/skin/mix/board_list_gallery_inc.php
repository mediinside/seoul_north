...
if($data_list_gallery_cnt > 0) {
	for ($i=0; $i<$data_list_gallery_cnt; $i++ ) {
			$jb_reg_date 				= date("Y.m.d", strtotime($data_list_gallery[$i]['jb_reg_date']));							//등록일
			$jb_mod_date 				= date("Y.m.d", strtotime($data_list_gallery[$i]['jb_mod_date']));							//수정일
			$jb_see 					= $C_Func->num_f($data_list_gallery[$i]['jb_see']);									//조회수 (3자리 단위 ',' 처리)
			$jb_comment_count 			= $C_Func->num_f($data_list_gallery[$i]['jb_comment_count']);  			//코멘트 수 (3자리 단위 ',' 처리)
			$jb_name 					= $data_list_gallery[$i]['jb_name'];
			...
			$jb_file_code				= $data_list_gallery[$i]['jb_file_code'];
			$jb_mb_id					= $data_list_gallery[$i]['jb_mb_id'];
			$jb_type					= $data_list_gallery[$i]['jb_type'];
			$jb_show					= $data_list_gallery[$i]['jb_show'];
			$jb_etc1					= $data_list_gallery[$i]['jb_etc1'];
			$jb_mb_level				= $data_list_gallery[$i]['jb_mb_level'];
			$jb_secret_check			= $data_list_gallery[$i]['jb_secret_check'];
			$jb_email					= $data_list_gallery[$i]['jb_email'];
			$jb_group					= $data_list_gallery[$i]['jb_group'];
			$jb_code					= "20";
			$jb_homepage				= $data_list_gallery[$i]['jb_homepage'];
			$jb_order					= $data_list_gallery[$i]['jb_order'];
			$jb_idx						= $data_list_gallery[$i]['jb_idx'];
			$jb_step					= $data_list_gallery[$i]['jb_step'];
			$jb_title 					= $C_Func->strcut_utf8($data_list_gallery[$i]['jb_title'], 43, true, "...");	//제목 (길이, HTML TAG제한여부 처리)
			$jb_content					= $C_Func->dec_contents_view($data_list_gallery[$i]['jb_content']);
			...
			$new_icon = $C_Func->new_icon(1, $data_list_gallery[$i]['jb_reg_date'], $new_image);
			//파일 이미지
